...
function User({
    ...
    const removeFromBallot = async () => {
        ...
        const currentBallots = profile.ballots || [];
        const newBallots = profile.ballots.filter((ballot) => ballot !== committee.id);
        const { data, error2 } = await supabase
            .from("faculty_profiles")
            .update({
                ballots: newBallots,
    ...
    const addToCommittee = async (user) => {
        let { data: committeeData, error2 } = await supabase
            .from("committees")
            .select("*")
            .eq("id", router.query.id);
        ...
        if (!committeeData[0].members) {
            ...
            const { data, error } = await supabase
                .from("committees")
                .update({ members: [user.employeeID] })
                .eq("id", router.query.id);
        } else {
            console.log("here");
            let umembers = committeeData[0].members;
            umembers.indexOf(user.employeeID) === -1
                ? umembers.push(user.employeeID)
