...
export default function Account({ session }) {
	...
	async function getProfile() {
		try {
			...
			let { data, error, status } = await supabase
				.from("profiles")
				.select(`username, avatar_url, rank, hamId, interested_committees, past_committees`)
				.eq("id", user.id)
				.single();
			if (error && status !== 406) {
				throw error;
	...
	const updateCommitteeMembers = async ({ interestedCommittees, pastCommittees }) => {
		const user = await getCurrentUser();
				...
				<div className="pr-4">
					<Button
						...
						onClick={() => {
							...
							updateCommitteeMembers({ interestedCommittees, pastCommittees });
						}}
						disabled={loading}
					>
