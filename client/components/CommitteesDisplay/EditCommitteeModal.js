...
export default function EditCommitteeModal({ modal, closeModal, committeeId, committeeName, committeeElected }) {
	...
	async function UpdateCommitteeName(name, committee_id) {
		if (name && committee_id) {
			const { error } = await supabase.from("committees").update({ display_name: name }).eq("id", committee_id);
			if (error) {
				console.error(error);
				return;
			}
	...
	async function UpdateCommitteeDescription(description, committee_id) {
		if (description && committee_id) {
			const { error } = await supabase.from("committees").update({ description: description }).eq("id", committee_id);
			if (error) {
				console.error(error);
				return;
			}
	...
	async function DeleteCommittee(committee_id) {
		if (committee_id) {
			const { error } = await supabase.from("committees").delete().eq("id", committee_id);
		}
		setCommiteeDeleted(commiteeDeleted.concat(<DeletedMessage />));
	}

	async function updateElectedStatus() {
		const { error } = await supabase.from("committees").update({ elected: electedChoice["value"] }).eq("id", committeeId);
		if (error) {
			console.error(error);
			return;
		}
	...
	return (
		<Modal dismissible={true} show={modal} onClose={closeModal}>
			<Modal.Header>{confirm ? "Success!" : `Edit Details`}</Modal.Header>
			<Modal.Body>
				{confirm ? (
					<div>
						<p>Edits Complete</p>
					...
					<EditCommitteeForm confirm={confirm} handleConfirm={() => setConfirm(true)} committeeId={committeeId}>
						{(formData, handleChange) => (
							<>
								<h1 style={{ fontSize: "25px", marginBottom: "0px", marginTop: "0px" }}>{`${committeeName} Committee `}</h1>
								{committeeElected ? (
									<p style={{ marginBottom: "15px" }}>(Elected Committee)</p>
								) : (
									<p style={{ marginBottom: "15px" }}>(Appointed Committee)</p>
								)}
								<Label htmlFor="name">New name of the Committee</Label>
								<TextInput
									value={formData["name"] || ""}
								...
								<Label htmlFor="description">Enter a new description for this committee</Label>
								<Textarea
								...
								<Label htmlFor="description">Set this committee to be marked as selected or appointed</Label>
								<Select options={ElectOptions} onChange={(choice) => setElectedChoice(choice)} />
								<Button onClick={() => updateElectedStatus()} style={{ marginBottom: "30px", marginTop: "5px" }}>
									Update
								</Button>
								{electedStatusChanged}

								<Button onClick={() => DeleteCommittee(committeeId)} style={{ background: "#CA2C2C", marginTop: "20px" }}>
									Delete Committee
								</Button>
								{commiteeDeleted}
							</>
